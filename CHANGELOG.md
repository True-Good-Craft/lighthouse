...
- Added `freshness_basis` to every CEO source state. Direct aggregate sources use `activity`, remain `freshness: "unknown"`, use `activity_only` when an activity watermark exists, and expose `data_through` only as that watermark. A single-component watermark is its latest trusted observation; a composite watermark is the conservative earliest required-component watermark; a day-bucket watermark is a bounded reporting timestamp rather than an event timestamp. Sparse silence is no longer mislabeled `fresh`, `stale`, or failed; a successful source with no history remains `unknown`/`source_history_missing`.
- Reserved `freshness_basis: "scheduled_probe"` and `fresh`/`stale` for `service_probes`. A successful query with no probe history is available/unknown with `probe_history_missing` and empty details; a partial active-target set is available/unknown with `probe_history_incomplete`, observed details, and the latest observed watermark; query failure alone is unavailable. A complete six-target set retains its oldest-required-target 36-hour fresh/stale rule. Exact latest-timestamp ties now resolve deterministically with failure before success and a stable row-ID tie-break, preventing a tied pass/fail pair from producing order-dependent status.
- Clarified that voluntary-inquiry windows count unique lead records by `created_at`; a later update is not counted as a new inquiry record and does not move that record into a later created-at window. Added the aggregate-only `details.voluntary_inquiry_records` diagnostic with `total_records`, `last_created_at`, and `last_updated_at`, plus the exact limitation `existing_lead_updates_increment_inquiry_windows: false`; it returns no lead PII, record identifiers, email, form content, or raw attribution. The strict schema now rejects non-null inquiry window totals whenever the inquiry source is unavailable.
- Preserved the strict `1.1` schema for rollback and made the canonical CEO diagnostic helper strictly validate either `1.1` or `1.2` while requiring metric-definition version `1.1`. Agent Smith `0.26.0` established deployed dual-version compatibility on 2026-08-27: merged `main` commit `5519764d959cf1d6a505280814c59e82854f9bda` passed CI run `33105121602` and deployment run `33105121605`, including Discord command registration, and the owner confirmed `/report` works against active Lighthouse CEO `1.1`. The final Lighthouse audit subsequently added the unavailable-source inquiry-count rejection. Deployed Smith accepts every valid `1.2` producer output and retains `1.1` rollback support, but exact validator parity requires a matching governed Agent Smith patch before Lighthouse production promotion.
- Preserved all routes, headers, authorization behavior, CORS, Worker configuration and bindings, D1 schema and storage semantics, retention, schedules, service-probe/canary requests, ingestion, external integrations, and secret requirements. No migration, secret operation, direct SQL, configuration change, or producer change is required.
- Corrected the production baseline from the separately approved 2026-08-27 receipts: release workflow run `33086080869` succeeded for merged `main` commit `4e80d65de01606c15100a99e7852ce19c5e6cd98`; Lighthouse `1.30.0` Worker version `ab29c0fb-ca9e-4074-a379-18d0943ec02c` is active at 100% under deployment `526f9fbc-6432-466c-9b39-2ae90b299cae`; and the approved CEO read occurred at `2026-08-27T15:08:12.012Z`. No build timestamp is inferred.
- This local 1.31.0 bundle made no production endpoint request, Cloudflare read, upload, deployment, traffic change, D1 mutation, secret operation, or external-system change. The production facts above are retained receipts from the separately approved prior audit and release steps; production remains on CEO contract `1.1` until a separately approved 1.31.0 promotion.
...
- Verified `lighthouse.buscore.ca` as an active Production Custom Domain rather than a Worker Route, verified the active `5 0 * * *` cron, and reconciled the deployed D1 and R2 binding identities.
- Pinned Wrangler to Cloudflare account `eb1a8dd5723031d94e57642e3eaaebda` and corrected the primary D1 metadata name from `buscore-lighthouse` to the control-plane name `lighthouse` while preserving binding `DB` and database ID `e46f2daa-7e97-45a3-9bf0-49003a42850c`.
- Removed the ambiguous direct `deploy` package command and added explicit `release:upload`, `release:status`, and `release:history` commands. No direct production-deploy script is provided; production promotion must pass through the validated workflow, which preserves dashboard-managed variables, enables Wrangler strict mode, and returns a machine-readable deployment receipt. Package repository metadata now points to `True-Good-Craft/lighthouse`.
- Made `.github/workflows/deploy.yml` manual-dispatch only, main-only, non-cancelling and serialized, with dependency installation, typechecking, the full test suite, strict production deployment, and an active-deployment JSON receipt in that order. The receipt is attempted whenever the deploy step ran, including an uncertain reported failure; it remains skipped when validation prevents deployment. The workflow relies on the pinned account rather than a second account-ID secret.
- Added deterministic release-control tests covering the pinned Cloudflare identity, D1 metadata, sole manual trigger, main guard, validation/deployment order, serialization, variable preservation, strict deployment, explicit operator commands, and canonical repository metadata.
- Added a rollback-safe operator procedure that requires an explicitly approved immutable version ID, preserves Wrangler's interactive confirmation, verifies the resulting traffic assignment by JSON status receipt, and keeps Git, settings, secrets, domains, D1, and migrations as separate rollback domains.
- Made standalone setup fail safer and more reproducibly: Node.js `20.18.1` or newer is required by the lockfile, dependency installation uses `npm ci`, new environments require an explicit non-production Wrangler configuration, migration examples use the stable `DB` binding, and new-environment migration/secret commands cannot silently inherit the pinned production target.
- Completed the mandatory external release-control prerequisite on 2026-08-26: changed the Cloudflare Workers Builds production Deploy command from `npx wrangler deploy` to `npx wrangler versions upload`, then reloaded the control plane and confirmed that both Deploy and Version commands are exactly `npx wrangler versions upload`. The approved dashboard action was limited to saving this configuration field and re-reading it; no build, upload, or deployment control was invoked.
- Recorded remaining operational gaps: persistent Worker logs and traces are disabled; the unused legacy secret names `DISCORD_WEBHOOK_URL` and `PRICE_GUARD_KEY` remain attached pending a separately approved decision; the GitHub deployment-token path is not proven by repository text; Cloudflare still displays the redirecting historical repository name `True-Good-Craft/buscore-lighthouse`; and the current lockfile install reports one low- and five high-severity npm audit findings whose production relevance and remediation have not yet been assessed. No automatic audit fix is authorized by this bundle.
- No Worker source, endpoint, response contract, auth behavior, storage schema or semantics, retention, schedule, ingestion, report metric, runtime integration, secret value, D1 resource, R2 bucket, custom-domain attachment, migration, or active production traffic changed in this local bundle. Production remains Worker version `ba611ac1-653d-47a2-a465-a85f4124b6b6` until a separately approved deployment occurs.
...
- Recorded the separately connected Cloudflare Workers Builds path: publishing the review branch automatically uploaded preview Worker version `33f4db25-9faf-435d-a83e-83d7d1c17eac` and created version/branch preview URLs. The Cloudflare check classified the upload as a preview and did not report an active-production promotion; active-production state was not independently control-plane verified. The checked-in GitHub Actions release gate did not govern this integration, so at that time a merge remained potentially production-deploying until the Cloudflare production-branch and deploy-command settings were explicitly verified or the owner accepted that consequence.
